...
const Test = () => {
    ...
    const [elements, setElements] = useState([{ id: "first-line", html: "" }])
    const refs = useRef([])
...
useEffect(()=>{
    ...
    const handleKeyDown = (index) => e => {
        if (e.keyCode == 13) {
            e.preventDefault()
            elements.splice(index + 1, 0, { id: nanoid(3), html: '' })
            setElements([...elements])
            setFocused(index+1)
        }
        if (e.keyCode == 8 && elements[index].html == "") {
